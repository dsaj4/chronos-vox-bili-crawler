...
class AsyncFileWriter:
    ...
    @classmethod
    def _get_lock(cls, file_path: str) -> asyncio.Lock:
        if file_path not in cls._locks:
            cls._locks[file_path] = asyncio.Lock()
        return cls._locks[file_path]

    @staticmethod
    def _load_existing_json_items(file_path: str) -> List[Dict]:
        ...
        utils.logger.warning(
            f"[AsyncFileWriter._load_existing_json_items] Could not fully parse existing JSON file, "
            f"starting fresh: {file_path}"
        )
        return []
    ...
    def _get_file_path(self, file_type: str, item_type: str) -> str:
        ...
        output_date = getattr(config, "OUTPUT_DATE_OVERRIDE", "") or utils.get_current_date()
        file_name = f"{self.crawler_type}_{item_type}_{output_date}.{file_type}"
    ...
    async def write_single_item_to_json(self, item: Dict, item_type: str):
        ...
        async with self._get_lock(file_path):
            existing_data = self._load_existing_json_items(file_path)
    ...
    async def generate_wordcloud_from_comments(self):
        ...
        try:
            ...
            output_date = getattr(config, "OUTPUT_DATE_OVERRIDE", "") or utils.get_current_date()
            words_file_prefix = f"{words_base_path}/{self.crawler_type}_comments_{output_date}"
