...
class BilibiliClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    def _parse_response_json(self, response: httpx.Response) -> Dict[str, Any]:
        ...
        try:
            ...
            utils.logger.error(
                ...
                f"status_code: {response.status_code}, response_text: {preview}"
            )
            raise DataFetchError(f"Failed to decode JSON, content: {preview}")
    ...
    async def request(self, method, url, **kwargs) -> Any:
        ...
        data: Dict = self._parse_response_json(response)
        if data.get("code") != 0:
    ...
    async def get_video_all_comments(
        ...
        total_count = 0
        sub_comment_count = 0
        while not is_end and total_count < max_count:
            comments_res = None
            ...
            remaining_total = max_count - total_count
            if remaining_total <= 0:
                break
            if len(comment_list) > remaining_total:
                comment_list = comment_list[:remaining_total]
            ...
            if is_fetch_sub_comments and total_count < max_count:
                for comment in comment_list:
                    if total_count >= max_count:
                        break
                    if sub_comment_count >= max_sub_comment_count:
                        break
                    ...
                    fetched_sub_comments = await self.get_video_all_level_two_comments(
                        video_id=video_id,
                        level_one_comment_id=comment_id,
                        order_mode=CommentOrderType.DEFAULT,
                        ps=10,
                        crawl_interval=crawl_interval,
                        callback=callback,
                        max_count=min(max_count - total_count, max_sub_comment_count - sub_comment_count),
                    )
                    sub_comment_count += fetched_sub_comments
                    total_count += fetched_sub_comments
    ...
    async def get_video_all_level_two_comments(
        ...
        while True:
            ...
            if max_count > 0:
                remaining = max_count - fetched_count
                if remaining <= 0:
                    break
            ...
            if callback:  # If there is a callback function, execute it
                await callback(video_id, comment_list)
            fetched_count += len(comment_list)
            await asyncio.sleep(crawl_interval)
